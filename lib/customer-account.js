...
export async function exchangeToken({ code, codeVerifier, redirectUri }) {
  ...
  // If configured as confidential client, use Basic auth; otherwise public client works with PKCE.
...
export async function customerApiFetch(accessToken, query, variables = {}) {
  ...
  if (!graphqlEndpoint) {
    throw new Error(
      `Customer API discovery missing graphql endpoint: ${JSON.stringify(customerApi)}`
    );
  }
